...
8. Daily step-driven active energy from a phone or watch is displayed on the Fitness screen but is never added to the budget in version 1.
...
18. Projected goal date: weeks to goal = (current trend weight - goal weight) / chosen weekly rate, from today. The UI shows the specific date ("Dec 18, 2026") under "Goal forecast" and adds "Estimate. Updates with each weigh-in." (Product decision 2026-09-02: a date, not a month; the estimate caveat carries the uncertainty.)
...
31. Goal direction is inferred from the two weights and never chosen. A goal weight under the latest weigh-in is a losing plan, one above it is a gaining plan, and no goal weight, no weigh-in, or the two the same is maintaining. There is no Lose / Maintain / Gain chooser anywhere, and no goal is stored: the two weights are the only record of it. The latest weigh-in rather than the trend weight (decision 19), so the direction agrees with the number the member is looking at. A goal weight that turns the direction around drops the stored goal rate, because a rate belongs to the direction it was picked under.
   Why: a product decision of 2026-09-02. A member who has typed both weights has already said which way they are going, and asking again is a second answer that can disagree with the first.

32. Activity goals: a day aims at 30 minutes of exercise and 8,000 steps, and both are editable (5 to 600 minutes, 1,000 to 50,000 steps). They are goals to fill a ring against and nothing else: neither one changes the calorie budget, and steps are not credited as energy (decision 8).
   Why: the WHO recommends 150 to 300 minutes of moderate-intensity aerobic activity a week for adults 18 to 64, or 75 to 150 minutes of vigorous-intensity, or an equivalent combination, [32] and 30 minutes a day is the plainest way to say the bottom of that range. For steps, a meta-analysis of 15 cohorts found mortality risk falling until 6,000 to 8,000 steps a day in adults 60 and over and until 8,000 to 10,000 in adults under 60, with no further benefit above those bands. [33] 8,000 is inside both, which is why it is the default rather than the 10,000 that came from a 1960s pedometer name. The step ring is drawn only once a device sends steps (Round 9); until then the goal is stored and not shown.

33. Decision 7's overlap rule, at the resolution the data has: a day's exercise credit is the larger of the manual total and the imported total, never their sum, and the same for its minutes. A manual entry carries a day and a length and no clock time, so whether two entries overlap cannot be asked of the data.
   Why: decision 7 says an overlapping manual entry and imported workout count once and the imported one wins. Taking the larger side keeps the half of that rule which matters, that the same run is never counted twice, and it settles the half that cannot be tested in a way that loses nothing: the imported figure wins whenever it is the larger, which is the case the rule exists for, and an afternoon of manual work no phone recorded is not thrown away because a ten-minute walk was imported. Adding the two would inflate the budget, and dropping the manual entries would silently discard something a member typed in.

34. Every metric a phone exports is stored, under the name the exporter used, whether or not anything draws it. The four the Fitness screen shows are steps, active calories, exercise minutes and resting heart rate; sleep, heart rate variability, oxygen, respiratory rate, blood pressure and anything else the export carries are written to the same table and shown nowhere yet. A reading in a unit Tare has no rule for is averaged over its day; a reading that is not a single number, such as a night's sleep in stages, keeps all its fields and is given a headline figure where one of its fields is an obvious headline.
   Why: a product decision of 2026-09-03, and the arithmetic of it. A reading dropped on the way in is gone: Apple Health keeps its own copy, but the export that carried it is not stored, so a screen written next year would have nothing to draw for the year before it. Storage is cheap and a year of every metric a watch records is a few megabytes. Averaging an unknown unit is the safer of the two guesses, because a rate added up reads as an absurdly large number while a total averaged reads as a small one, and the second is easier to recognise as wrong.

35. A sync may reach back one year before the day the account was made. Anything older is refused and counted, never stored, and the window is anchored to the account rather than to today.
   Why: a health app is worth having because it knows what last year looked like, so a first sync from a phone with years on it should bring a year of it. Anchoring to the account rather than to today means a member who joins and then goes offline for a month still syncs every day of that month, while a member who joined this morning cannot import a decade of somebody else's history. Refused rather than an error, because an export reaching further back than the window is what a first sync looks like and is not a mistake.

36. Ingest never refuses a reading for looking wrong. A figure past what a body does is stored and marked: a workout with a pace no one runs, more than 100,000 steps in a day, a resting heart rate outside 25 to 150, a weight outside 20 to 400 kg. Only a reading that cannot be read at all is skipped, and the first one is named in that sync's record. A weight outside the range is the one exception to being used as well as stored: it lands in the fitness table and not in the weigh-in log.
   Why: the phone is reporting what it measured, and Tare is not the arbiter of whether somebody's morning happened. A flag says the number looked odd and leaves the number there; a refusal loses it and tells the member nothing. The weigh-in log is the exception because it is what the resting energy estimate, the trend weight and the budget are all read from (decisions 1, 2, 19), so a scale that read five kilograms would move every number on the Targets screen rather than only look wrong on one tile.

37. An export from a phone's own exporter is bucketed into days by the wall clock the phone wrote, offset included; an export from the Health Connect bridge arrives in UTC and is converted with the member's own time zone first. Health Connect has no daily exercise total, so its exercise minutes are the lengths of its own sessions added up, and they are filed under the same metric name the iPhone dialect uses.
   Why: an evening walk in the west of the United States is stamped after midnight in UTC, so bucketing it without converting would file it under tomorrow and put the steps on the wrong day of the ring. The iPhone exporter already writes the phone's own wall time, which is the same answer arrived at earlier. Filing Health Connect's minutes under the iPhone name means one tile reads one metric whichever phone a member carries, at the cost of an Apple word in a column nobody sees.
...
30. Federal Trade Commission. Health Breach Notification Rule, final rule of 2024-04-26. https://www.ftc.gov/legal-library/browse/rules/health-breach-notification-rule and https://www.ftc.gov/news-events/news/press-releases/2024/04/ftc-finalizes-changes-health-breach-notification-rule. Accessed 2026-09-01. The rule applies to "health apps and similar technologies not covered by HIPAA" and to vendors of personal health records. The HHS guidance page on HIPAA and health apps (https://www.hhs.gov/hipaa/for-professionals/privacy/guidance/access-right-health-apps-apis/index.html) returned 403 from every route, so the HHS half is UNVERIFIED. Whether a self-hosted private instance falls under the FTC rule is this document's reading, not the FTC's words.
...
32. World Health Organization. WHO guidelines on physical activity and sedentary behaviour. 2020. https://www.who.int/publications/i/item/9789240015128, guideline text read at https://www.ncbi.nlm.nih.gov/books/NBK566046/. Accessed 2026-09-02. Adults aged 18 to 64: at least 150 to 300 minutes of moderate-intensity aerobic physical activity a week, or at least 75 to 150 minutes of vigorous-intensity, or an equivalent combination.
33. Paluch AE, Bajpai S, Bassett DR, et al. Daily steps and all-cause mortality: a meta-analysis of 15 international cohorts. Lancet Public Health, 2022;7(3):e219-e228. https://doi.org/10.1016/S2468-2667(21)00302-9, read at https://pmc.ncbi.nlm.nih.gov/articles/PMC9289978/. Accessed 2026-09-02. Progressively lower mortality risk until 6,000 to 8,000 steps a day in adults 60 and over and until 8,000 to 10,000 steps a day in adults under 60 (interaction by age p = 0.012).
