...
36. Ingest never refuses a reading for looking wrong. A figure past what a body does is stored and marked: a workout with a pace no one runs, more than 100,000 steps in a day, a resting heart rate outside 25 to 150. Only a reading that cannot be read at all is skipped, and the first one is named in that sync's record. A scale reading is stored as a metric and nothing else: an export never writes a weigh-in, so nothing a phone sends can move the Targets screen.
   Why: the phone is reporting what it measured, and Tare is not the arbiter of whether somebody's morning happened. A flag says the number looked odd and leaves the number there; a refusal loses it and tells the member nothing. Weigh-ins are kept out of it because the log is what the resting energy estimate, the trend weight and the budget are all read from (decisions 1, 2, 19), and those belong to what the member typed rather than to what a scale posted.
